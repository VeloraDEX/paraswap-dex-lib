...
implementation phase. Phases 1, 2, 2a, 2b, 2c, 2d, 2e, 2f, and 3 are
complete; Phase 4 runtime bridge work remains out of scope for this document.
...
- Pro methods remain selector-only relative to regular In/Out inputs. Phase 2f
  ...
  Augustus V6 ABI.
- Phase 3 direct-builder tests should not call `GenericCalldataDiff` for direct
  methods because it is intentionally generic-method-only. Reuse its raw
  selector/byte-diff pattern or extract a small direct-safe helper if direct
  calldata diagnostics need more than exact hex equality.
...
## Phase 3: Direct Builder Fixture Parity

This section plans the Go direct resolved boundary after generic
`BuildTransactionFromResolved` parity. Phase 3 proves that Go can assemble
Augustus V6 direct-method calldata from committed resolved direct fixtures whose
`input.params` already contain DEX-specific encoded params.

### Current Status

- Phase 3 is complete.
- The fixture manifest already exposes:
  - `BucketPhase3DirectSuccess`: 10 fixtures
  - `BucketPhase3DirectNegative`: 3 fixtures
- `resolved.DirectBuildInput.Params` is `json.RawMessage`, so non-array params
  fail inside direct boundary validation with the TypeScript-compatible
  `direct params must be an array` error.
- Generic success parity is complete and should remain unchanged. Phase 3 must
  not relax generic executor guards or alter generic fixture buckets.
- The direct fixtures carry pre-encoded DEX params produced by the TypeScript
  direct DEX encoder path. Phase 3 does not implement Go-side DEX encoders.

### Phase Boundary

Phase 3 owns only the Go direct resolved boundary:

- `resolved.BuildDirectTransactionFromResolved`
- direct-method validation
- direct params passthrough for returned `params`
- direct params coercion for Augustus V6 ABI packing
- direct `TxObject` assembly
- direct success and direct-only negative fixture parity

Phase 3 does not implement:

- Go-side direct DEX encoders or `getDirectParamV6`
- public builder route planning/preprocessing
- generic executor bytecode changes
- runtime TypeScript-to-Go bridge behavior
- new direct fixtures unless they are generated by the shared TypeScript
  fixture generator and committed through the existing manifest

Direct fixtures already carry the DEX-specific encoded params payload in
`input.params`. Phase 3 verifies that the Go boundary produces identical
Augustus V6 calldata when given that payload as input.

### Required Fixture Gates

Direct success fixtures:

| Fixture                       | Method                           | Side |
| ----------------------------- | -------------------------------- | ---- |
| `uniswap-v2-sell`             | `swapExactAmountInOnUniswapV2`   | SELL |
| `uniswap-v2-buy`              | `swapExactAmountOutOnUniswapV2`  | BUY  |
| `uniswap-v3-sell`             | `swapExactAmountInOnUniswapV3`   | SELL |
| `uniswap-v3-buy`              | `swapExactAmountOutOnUniswapV3`  | BUY  |
| `balancer-v2-sell`            | `swapExactAmountInOnBalancerV2`  | SELL |
| `balancer-v2-buy`             | `swapExactAmountOutOnBalancerV2` | BUY  |
| `curve-v1-sell`               | `swapExactAmountInOnCurveV1`     | SELL |
| `curve-v2-sell`               | `swapExactAmountInOnCurveV2`     | SELL |
| `lite-psm`                    | `swapExactAmountInOutOnMakerPSM` | SELL |
| `augustus-rfq-try-batch-fill` | `swapOnAugustusRFQTryBatchFill`  | SELL |

Direct negative fixtures:

| Fixture                       | Expected error focus                                      |
| ----------------------------- | --------------------------------------------------------- |
| `unsupported-direct-method`   | unsupported direct contract method                        |
| `invalid-direct-side`         | direct side must be SELL or BUY                           |
| `direct-side-method-mismatch` | direct method side mismatch for side-derived method names |

Phase 3 tests must run all direct-only negative fixtures with exact
`expectedError` parity.

### Public Interfaces

Add direct-specific Go interfaces without expanding the generic builder API:
...
`ethabi` is the local alias for
`github.com/ethereum/go-ethereum/accounts/abi`, matching the existing
`resolved.BuildDeps` type.

Do not reuse `BuildDeps` for direct unless implementation proves the extra
generic-only fields do not leak into direct tests. Direct only needs the
Augustus V6 ABI.

Add a test helper in `internal/resolvedtest`, for example
`BuildDirectDepsFromFixtureInput(input resolved.DirectBuildInput)`, that loads
the cached embedded Augustus V6 ABI. It should not invent network, executor, or
wrapped-native context for direct tests.

### Validation Order And Error Strings

The canonical TypeScript source is
`src/generic-swap-transaction-builder/resolved/build-transaction.ts`.
Phase 3 must match `validateDirectBuildInput` call order:

1. `validateSupportedDirectContractMethod(input.contractMethod)`
2. `validateDirectSide(input.side)`
3. `validateDirectSideContractMethod(input.contractMethod, input.side)`
4. `validateDirectTopLevelFields(input)`

Exact committed error strings:

- `unsupported direct contract method for resolved build: ${contractMethod}`
- `direct side must be SELL or BUY: ${side}`
- `direct contract method ${contractMethod} is inconsistent with side ${side}; expected ${expectedSide}`

`validateDirectTopLevelFields` order:

1. `params` must be an array: `direct params must be an array`
2. `userAddress` lowercase address
3. `augustusV6Address` lowercase address
4. `srcToken` lowercase address
5. `srcAmount` decimal amount string
6. `minMaxAmount` decimal amount string
7. `gas.gasPrice` decimal amount string, if present
8. `gas.maxFeePerGas` decimal amount string, if present
9. `gas.maxPriorityFeePerGas` decimal amount string, if present

Phase 3 should change `DirectBuildInput.Params` from `[]json.RawMessage` to
`json.RawMessage` and parse it inside direct validation. This is required for
exact TypeScript parity on non-array params: TypeScript currently throws
`direct params must be an array` for `null` and `"0x"` params in
`tests/generic-swap-transaction-builder/resolved/build-direct-transaction.test.ts`.
Leaving `Params` as `[]json.RawMessage` would let Go JSON unmarshaling fail
before the boundary can return the committed validation message.

Add focused mutation tests, matching TypeScript unit-test messages where
possible, for malformed direct top-level fields even though the committed
direct negative fixtures only cover method/side validation.

### Direct Method Set

Mirror `src/generic-swap-transaction-builder/dex-encoder/direct-methods.ts`.
...
Side-derived methods:

- SELL: `swapExactAmountInOnUniswapV2`
- BUY: `swapExactAmountOutOnUniswapV2`
- SELL: `swapExactAmountInOnUniswapV3`
- BUY: `swapExactAmountOutOnUniswapV3`
- SELL: `swapExactAmountInOnBalancerV2`
- BUY: `swapExactAmountOutOnBalancerV2`
- SELL: `swapExactAmountInOnCurveV1`
- SELL: `swapExactAmountInOnCurveV2`

No fixed side in the method name:

- `swapOnAugustusRFQTryBatchFill`
- `swapExactAmountInOutOnMakerPSM`

Those two methods accept side from `input.side` for tx value calculation but do
not fail side/method consistency validation when side is SELL or BUY.

### Params Handling

TS returns `input.params` directly in the direct output and also passes that
same array to ethers for ABI encoding. Go should preserve that split:

- Return `Params` as a JSON-shaped `[]any` decoded from `input.Params`, so it
  deep-equals fixture `expectedParams`.
- Separately coerce `input.Params` into Go ABI values for
  `method.Inputs.Pack(...)`.

Do not return Go ABI-native values such as `common.Address`, `*big.Int`, byte
slices, or structs in `DirectBuildOutput.Params`; that would diverge from
fixture `expectedParams` and from TypeScript's direct boundary output.

JSON-shape decoding must preserve fixture JSON types verbatim:

- string-encoded amounts stay Go strings; do not convert output params to
  `*big.Int`, `json.Number`, or numeric Go types
- nested arrays decode to `[]any` recursively
- bytes fields stay as the fixture's `0x`-prefixed strings
- committed direct fixtures use string-encoded uint values exclusively; do not
  introduce numeric JSON decoding that routes through `float64`

The decoded `[]any` returned in `DirectBuildOutput.Params` should be a fresh
slice derived from `input.Params`; it does not alias `input.Params`, which is
raw JSON bytes. Callers may mutate the returned params without mutating the
input.

Recommended implementation shape:

- `ParseDirectParamsForOutput(params json.RawMessage) ([]any, error)`
- `CoerceDirectParamsForABI(contractMethod string, params []json.RawMessage, method abi.Method) ([]any, error)`
- `encodeDirectCalldata(input DirectBuildInput, params []json.RawMessage, abi *abi.ABI) (HexBytes, error)`

ABI coercion can be method-specific for Phase 3. Prefer explicit typed structs
over a broad dynamic ABI reflection layer unless the dynamic layer is clearly
smaller and thoroughly tested. The committed direct fixtures cover only the 10
methods above; do not implement callback or unrelated ABI methods.

Method-specific structs should match the Augustus V6 ABI tuple fields exactly:

- UniswapV2 tuple: `srcToken`, `destToken`, `fromAmount`, `toAmount`,
  `quotedAmount`, `metadata`, `beneficiary`, `pools`
- UniswapV3 tuple: `srcToken`, `destToken`, `fromAmount`, `toAmount`,
  `quotedAmount`, `metadata`, `beneficiary`, `pools`
- BalancerV2 tuple: `fromAmount`, `toAmount`, `quotedAmount`, `metadata`,
  `beneficiaryAndApproveFlag`; top-level args also include `partnerAndFee`,
  `permit`, and `data`
- CurveV1 tuple: `curveData`, `curveAssets`, `srcToken`, `destToken`,
  `fromAmount`, `toAmount`, `quotedAmount`, `metadata`, `beneficiary`
- CurveV2 tuple: `curveData`, `i`, `j`, `poolAddress`, `srcToken`,
  `destToken`, `fromAmount`, `toAmount`, `quotedAmount`, `metadata`,
  `beneficiary`
- Maker PSM tuple: `srcToken`, `destToken`, `fromAmount`, `toAmount`, `toll`,
  `to18ConversionFactor`, `exchange`, `gemJoinAddress`, `metadata`,
  `beneficiaryDirectionApproveFlag`; top-level args also include `permit`
- Augustus RFQ try-batch-fill: quote data tuple (`fromAmount`, `toAmount`,
  `wrapApproveDirection`, `metadata`, `beneficiary`), `orders` tuple array, and
  `permit`

The embedded Augustus V6 ABI remains the source of truth for field names,
ordering, and widths. The lists above are a checklist to prevent omissions in
the Phase 3 port.

Scalar coercion rules must be explicit and tested against
`go-ethereum/accounts/abi`:

- `address` -> `common.Address`
- `bytes` -> `[]byte`
- `bytes32` -> `[32]byte`
- `uint8` -> `uint8` (for example RFQ `wrapApproveDirection`)
- `uint16`, `uint32`, and `uint64` -> exact-width unsigned Go integers when
  the ABI uses those widths
- `uint128`, `uint160`, `uint256`, and other widths above 64 bits -> the
  ABI-accepted big integer representation, normally `*big.Int`
- signed integer fields, if a future direct fixture introduces them, must be
  handled deliberately from ABI type width instead of falling through a generic
  decimal parser

Do not parse every numeric string to `*big.Int` blindly. Width-sensitive ABI
fields such as RFQ `uint8 wrapApproveDirection` and `uint128 expiry` need the
Go type accepted by the ABI packer for that exact field.

Augustus RFQ is the hardest direct method because it requires nested typed
structs: an outer order-info tuple containing an inner order tuple plus
signature/fill/permit fields. Implement RFQ after the flatter direct methods
have proven the params-output and scalar-coercion path.

For each method, add a direct params unit test that packs the committed
fixture's `input.params` and compares to `expectedTx.data`. This catches tuple
field order mistakes before the public boundary test.

### TxObject Rules

Direct tx assembly mirrors TypeScript `buildDirectTxObject`:

- `from` = `input.userAddress`
- `to` = `input.augustusV6Address`
- `data` = Augustus V6 ABI encoding of `input.contractMethod` with coerced
  params
- `value`:
  - if `input.srcToken == NativeTokenAddress` and side SELL: `input.srcAmount`
  - if `input.srcToken == NativeTokenAddress` and side BUY: `input.minMaxAmount`
  - otherwise: `0`
- gas fields pass through exactly when present

Reuse the generic tx-value logic only if it does not require constructing a
fake generic `BuildInput`. A small direct-specific helper is clearer.

### Calldata Diagnostics

`GenericCalldataDiff` is generic-method-only and should not decode direct
methods. Phase 3 should ship with raw-byte direct calldata diagnostics:

- selector value and method name looked up from the embedded Augustus V6 ABI
- total byte lengths
- first differing byte offset
- short lowercase hex windows around the first differing byte

Do not decode direct calldata as generic calldata. If a direct decoded diff is
needed later, it must be method-specific or ABI-driven and tested against at
least one fixture per heterogeneous param shape. Method-specific decoded direct
diffs are deferred unless a Phase 3 fixture failure proves raw-byte diagnostics
are insufficient.

### Implementation Order

1. Add `DirectBuildDeps`, `DirectBuildOutput`, and
   `BuildDirectTransactionFromResolved`.
2. Add direct method constants, supported-method predicate, and side mapping.
3. Add direct validation in TypeScript call order, with exact committed
   negative-fixture error strings.
4. Add direct params output parsing that preserves fixture JSON shape.
5. Add method-specific params-to-ABI coercion for the 10 committed direct
   methods.
6. Add direct calldata encoding and tx-object assembly.
7. Add direct success fixture tests over `BucketPhase3DirectSuccess`, comparing
   `contractMethod`, `params`, and `txObject`.
8. Add direct negative fixture tests over `BucketPhase3DirectNegative`, matching
   exact `expectedError`.
9. Add focused mutation tests for direct top-level validation and native BUY
   tx-value behavior. Native SELL value is covered by the committed
   `uniswap-v2-sell` fixture.
10. Re-run the full Phase 3 verification gate.

### Implementation Tasks

| Status | Task                               | Notes                                                                                |
| ------ | ---------------------------------- | ------------------------------------------------------------------------------------ |
| Done   | Add direct deps/output boundary    | Added `BuildDirectTransactionFromResolved` without changing generic output.          |
| Done   | Port direct method validation      | Supported-method set, side validation, side/method consistency, top-level fields.    |
| Done   | Preserve params output shape       | Decodes raw params to JSON-shaped `[]any` for fixture equality.                      |
| Done   | Add ABI coercion for direct params | Added ABI-driven coercion for all 10 committed direct methods.                       |
| Done   | Encode direct calldata             | Packs coerced params with embedded Augustus V6 ABI and assembles `TxObject`.         |
| Done   | Add direct success fixture tests   | Runs all `BucketPhase3DirectSuccess` fixtures against expected params and tx object. |
| Done   | Add direct negative fixture tests  | Runs all `BucketPhase3DirectNegative` fixtures with exact `expectedError` parity.    |
| Done   | Add focused direct unit tests      | Covers top-level validation, native BUY value, and per-method params packing tests.  |
| Done   | Run Phase 3 gates                  | Go tests/vet/format plus fixture and resolved Jest checks passed on 2026-05-15.      |

### Acceptance Criteria

Phase 3 is complete when:

- All 10 `BucketPhase3DirectSuccess` fixtures pass with exact
  `contractMethod`, `params`, and `txObject` parity.
- All 3 `BucketPhase3DirectNegative` fixtures pass with exact `expectedError`
  parity.
- Direct params returned by Go remain JSON-shaped and fixture-comparable; ABI
  native values are internal only.
- Direct calldata is packed through the embedded Augustus V6 ABI, not by
  concatenating calldata strings manually.
- Direct native-source SELL and BUY value derivation is tested.
  `uniswap-v2-sell` covers native-source SELL through a committed fixture;
  native-source BUY must be covered by a focused mutation test because there is
  no committed native BUY direct fixture.
- All Phase 2a-2f tests continue to pass without modification, including:
  generic real-builder parity across `BucketPhase2GenericSuccess`, Pro
  selector-body parity tests, Executor02 BUY and Executor03 non-BUY fence
  rejection tests, and generic negative fixtures.
- No Go-side DEX encoder, route planning, runtime bridge, or generic executor
  guard changes are introduced.
- `go test ./go/...` passes.
- `go vet ./go/...` passes.
- `gofmt -s -l go/` produces no output.
- `yarn fixtures:check` passes.
...
### Exit Notes For Runtime-Bridge Planning

When Phase 3 lands, capture:

- Final direct fixture count and method distribution.
- The exact `BuildDirectTransactionFromResolved` API shape.
- Whether direct calldata diagnostics stayed raw-only or gained method-specific
  decoded output.
- Any method-specific ABI coercion helpers that could be reused by future
  direct DEX encoder ports.
- Phase 3 chose dynamic ABI-driven direct-param coercion instead of per-method
  typed structs. That keeps fixture coverage compact, but reusability for
  future direct DEX encoder ports is TBD and should be re-evaluated before
  exporting those helpers as a broader API.
- Confirmation that runtime use still requires Go-side direct DEX encoders or a
  separate TS/Go boundary for producing direct params.
- Any remaining validation gaps that are covered by TS unit tests but not by
  committed direct fixtures.
