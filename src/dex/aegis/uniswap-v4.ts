...
export class Aegis extends SimpleExchange implements IDex<UniswapV4Data> {
  ...
  public static dexKeysWithNetwork: { key: string; networks: Network[] }[] = [
    { key: 'Aegis', networks: Object.keys(AegisConfig.Aegis).map(Number) },
  ];
  ...
  constructor(
    ...
    protected routerAddress = AegisConfig[dexKey][network].router,
    protected quoterAddress = AegisConfig[dexKey][network].quoter,
    protected poolManagerAddress = AegisConfig[dexKey][network].poolManager,
  ) {
  ...
  async getPricesVolume(
    ...
    const pricesPromises = availablePools.map(async poolId => {
      ...
      if (poolState !== null && poolState.isValid) {
        // Check if this is a dynamic fee pool
        const isDynamicFee = BigInt(pool.key.fee) === 0x800000n;

        if (isDynamicFee) {
          // For dynamic fee pools, use the quoter contract which correctly handles dynamic fees
          this.logger.info(
            `${this.dexKey}-${this.network}: pool ${poolId} is dynamic fee pool, using quoter contract for price calculation`,
          );
          ...
          // For static fee pools, use local calculation
  ...
  async getTopPoolsForToken(
    ...
    const res = await queryAvailablePoolsForToken(
      this.dexHelper,
      this.logger,
      this.dexKey,
      AegisConfig[this.dexKey][this.network].subgraphURL,
      _tokenAddress,
      limit,
      AegisConfig[this.dexKey][this.network].supportedHooks,
    );
