...
export async function queryAvailablePoolsForToken(
  ...
  const poolsQuery = `query ($token: Bytes!, $count: Int, $hooks: [Bytes!], $minTVL: Int!) {
    pools0: pools(
        where: { token0: $token, hooks_in: $hooks, liquidity_gt: 0, totalValueLockedUSD_gte: $minTVL }
        orderBy: volumeUSD
        orderDirection: desc
        first: $count
      ) {
    ...
    pools1: pools(
      where: { token1: $token, hooks_in: $hooks, liquidity_gt: 0, totalValueLockedUSD_gte: $minTVL }
      orderBy: volumeUSD
      orderDirection: desc
      first: $count
    ) {
