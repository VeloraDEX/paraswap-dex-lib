...
class UniswapV4PoolMath {
  ...
  swapFromEvent(
    ...
    // For dynamic fee pools (fee = 0x800000), use the swap fee from the event
    const isDynamicFee = BigInt(poolState.fee) === 0x800000n;

    let swapFee: bigint;
    if (isDynamicFee) {
      // Use the fee from the swap event for dynamic fee pools
      swapFee = newSwapFee;
    } else {
      // Use calculated fee for static fee pools
