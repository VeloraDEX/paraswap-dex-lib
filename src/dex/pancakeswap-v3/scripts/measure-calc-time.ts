...
const logger = getLogger('PancakeSwapV3Benchmark');
const runsNumber = 1000;
const network = Network.MAINNET;
const dexHelper = new DummyDexHelper(network);
const dex = new PancakeswapV3(network, 'PancakeswapV3', dexHelper);
...
async function benchmarkPair(pair: (typeof pairs)[0], blockNumber: number) {
  ...
  for (const p of poolEntries) {
    ...
    logger.info(
      `  fee=${p.key}: ${ticks} ticks, liquidity=${p.state.liquidity}`,
    );
  }
  logger.info('');
  ...
  for (const pool of poolEntries) {
    ...
    for (let i = 0; i < runsNumber; i++) {
      ...
      pancakeswapV3Math.queryOutputs(pool.state, amounts, zeroForOne, side);
  ...
  if (nativeAddonAvailable) {
    logger.info('  --- Rust ---');
    for (const pool of poolEntries) {
      const handle = createRustHandle(pool.state, 'pancakeswap_v3');
      if (!handle) {
      ...
      // Correctness
      const jsResult = pancakeswapV3Math.queryOutputs(
        pool.state,
        amounts,
        zeroForOne,
        side,
