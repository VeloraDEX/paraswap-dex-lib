...
- **[2026-08] BalancerV2 `getDexParam` without the optimizer**: `getPricesVolume` returns `data = { poolId }`; the `swaps: [{ poolId, amount }]` array that the encoding path iterates is normally built by `balancerV2Merge` (`src/dex/balancer-v2/optimizer.ts`), which the new standalone API never runs — hence `TypeError: data.swaps is not iterable`. Fixed by normalizing in `getDexParam` (`toOptimizedData`): when `data.swaps` is absent, rebuild the single-swap shape from `data.poolId` plus `getDexParam`'s own `srcAmount` (SELL) / `destAmount` (BUY) — the same amounts the optimizer would have used. The amount can't be added at pricing time because it depends on the route split. The discriminator has to test `data.swaps?.length`, not just presence: an empty `swaps` array ABI-encodes a no-op `batchSwap` — valid calldata that swaps nothing — instead of failing the request. Same class of bug applies to any dex whose optimizer reshapes `data`.
...
- **[2026-09] ERC4626 vaults that don't hold their own `asset`**: `ERC4626EventPool` derives `totalAssets` from `asset` transfers into the vault, which only holds for vaults custodying the asset directly. sftUSD forwards deposited ftUSD straight into ftftUSD and reports `totalAssets` as its ftftUSD balance, so relayer-fee flows (extra asset pulled in on deposit, `Withdraw.assets` net of the fee on redeem) made the tracked `totalAssets` creep upwards. Config now takes an optional `backingToken`: the pool follows that token's transfers in and out of the vault and leaves `totalAssets` alone on `Withdraw`. Note the erc4626 event test's `fetchPoolState` returns the event-derived state when one exists, so it compares state to itself — `STRICT_STATE_CHECK` opts a vault into a real on-chain comparison (sUSDe can't join: its `totalAssets` vests with time, not logs).

- **[2026-09] Executor overwrites the encoded amount on BUY**: the executor locates the word to insert the amount it holds by searching the dex calldata for the quoted `srcAmount`. ERC4626's BUY calldata encodes only the exact output amount (`mint`/`withdraw`), so for a vault trading at exactly 1:1 (srcAmount === destAmount) the search lands on that word and the swap consumes the whole slippage-buffered input — an e2e BUY of 1e6 spent 1.01e6. Other vaults are saved only because the search misses and falls back to an out-of-range position. Fix: append a scratch word to the BUY calldata (trailing bytes are ignored when the call is decoded) and set `insertFromAmountPos` to it. Setting `swappedAmountNotPresentInExchangeData` instead is not equivalent — it zeroes the flag and `fromAmountPos`, which reverted the `mint` direction.
